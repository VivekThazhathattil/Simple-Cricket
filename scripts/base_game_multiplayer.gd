...
func _ready():	
	print("base_game_multiplier path = " + str(self.get_path()))
...
#	var info = multiplayer.self_data
#	new_player.init(info.name, info.position, false)

func _on_player_disconnected(id):
	get_node("/root/menu").change_scene('res://texture/multiplayer_menu.tscn')
...
func _set_toss_decision():
	...
	print("base_game_multiplier:38: coin_toss._player_batting = " + str($coin_toss._player_batting))
	print("base_game_multiplier:39: _player_batting = " + str(_player_batting))
	_update_labels()
...
func _check_if_won():
	if _player_batting and player_score > opp_score and num_sides_batted_so_far == 2:
		print("base_game_multiplier:59: player has scored more than the opponent")
		_match_over = true
		_end_match()
	elif not _player_batting and opp_score > player_score and num_sides_batted_so_far == 2:
		print("base_game_multiplier:62: opponent has scored more than the player")
		_match_over = true
		_end_match()
func _main_handler():
	if _player_batting:
